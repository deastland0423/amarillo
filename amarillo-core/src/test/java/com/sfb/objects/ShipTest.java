...
public class ShipTest {
	...
	public void initTest() {
		...
		// Verify lwarp
		assertEquals(15, testShip.getPowerSystems().getAvailableLWarp());

		// Verify impulse
		assertEquals(5, testShip.getPowerSystems().getAvailableImpulse());

		// Do some damage to the impulse.
		assertTrue(testShip.getPowerSystems().damageImpulse());

		// Check that the impulse power is reduced.
		assertEquals(4, testShip.getPowerSystems().getAvailableImpulse());

		// Repair impulse
		assertTrue(testShip.getPowerSystems().repairImpulse(1));
		// Verify impulse is back to original value.
		assertEquals(5, testShip.getPowerSystems().getAvailableImpulse());

		// Try to repair again, and it fails because there are no undamaged systems.
		assertFalse(testShip.getPowerSystems().repairImpulse(1));
